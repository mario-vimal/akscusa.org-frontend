...
/** Whether Open Library holds any record at all for this ISBN. */
const hasRecordFor = async (book) =>
  Object.keys(await fetchRecord(book.isbn)).length > 0;
...
async function main() {
  ...
  for (const book of books) {
    ...
    const details = coversOnly ? undefined : await detailsFor(book);
    if (details && details.names.length > 0) filled += 1;
    ...
    if (coverOutcome === "unavailable") {
      ...
      // rather than merely lacking artwork. Under --covers-only no record was
      // fetched, so one is fetched here — for this book alone — rather than
      // assuming there is one and reporting the wrong thing.
      const hasRecord = details ? details.hasRecord : await hasRecordFor(book);

      (hasRecord ? noCover : noOpenLibraryEntry).push(label);
    }
  }
