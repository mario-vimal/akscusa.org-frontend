...
describe("resolveReadings", () => {
  ...
  it("omits a draft book without losing the reference that distinguishes it from articles", () => {
    ...
    expect(readingsOfBook.size).toBe(0);
    expect(readings[0].book).toBe("why-were-women-enslaved");
  });

  it("handles empty collections", () => {
    const resolved = resolveReadings<Book, Reading>([], [], [], (r) => r.book);
    expect(resolved.bookOfReading.size).toBe(0);
    expect(resolved.readingsOfBook.size).toBe(0);
  });
});
