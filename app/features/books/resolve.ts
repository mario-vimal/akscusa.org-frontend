 ...
 *
 * Kept apart from `queries/books.ts` so it can be tested without `astro:content`,
...
/**
 * An ISBN names one edition, so two book entries claiming it are rejected.
 * The clash is checked across drafts too, so withholding one cannot hide it.
 *
...
export interface Resolution<B, R> {
  /** The published book each reading points at, keyed by reading id. */
 ...
 * not: no book is joined, but the reading retains its reference so presenters
 * can distinguish an unpublished book from an article sitting. A draft is
 * unfinished, not missing, and toggling one in the CMS must not take down a
 * build that succeeds locally, where drafts are visible.
 */
